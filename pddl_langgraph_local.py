"""
Local-model friendly PDDL planner with tool-style calls, validation, and retries.

The agent is instructed to use tools, but when JSON tool calls are missing we
parse text (including code fences or XML-ish wrappers) into a tool call and
directly append the normalized plan line. Tools are still surfaced to the model
for naming/argument guidance.
"""
import argparse
...
def coerce_tool_call(response: AIMessage, tool_meta: Dict[str, Dict[str, Any]]) -> Tuple[Dict[str, Any], str]:
    """Return a tool-call dict and normalized plan line."""
    ...
    if parsed is None:
        ...
        # Simple line fallback: first non-empty line, strip fences/tags/backticks/parentheses.
        line = ""
...
def run_planner(
    ...
    def agent(messages: List, temp: float, seed: Optional[int]):
        # Local model; adjust temperature per step and ignore seed if not supported.
        step_llm = llm.__class__(
        ...
        return step_llm.invoke(messages)
    ...
    def attempt_one(
        ...
        if feedback:
            messages.append(HumanMessage(content=f"Validator feedback:\n{feedback}"))

        plan_lines: List[str] = prefix_lines.copy()
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        val_result, plan_lines = attempt_one(
            attempt_idx=attempt,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Local-model planner (tool-call style, direct append).")
